"""Ordered runner for the case-attribute analysis pipeline.
...
STEPS: list[tuple[str, str]] = [
    ("01", "01_describe.py"),
    ("02", "02_analyze_relevance.py"),
    ("03", "03_powerlaw_distributions.py"),
]
...
def parse_args(argv: List[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the case-attribute analysis pipeline (01->02->03)"
    )
...
def main(argv: List[str] | None = None) -> None:
    ...
    for step_id, filename in selected:
        ...
        print(f"$ python analyses/case_attributes/{filename} {' '.join(base_argv)}")
